...
CREATE TABLE outbox_authority (
  ...
  -- ATTEMPT is admitted by the domain so the freeze guard below can be written
  -- now and be inert on arrival. NOTHING IN THIS RELEASE CAN SET IT. The
  -- LEGACY -> ATTEMPT transition belongs to the activation capability, after
  -- the attempt table and attempt-aware writers exist; shipping it here would
  -- be a loaded gun capable of freezing legacy writes while no other authority
...
WHEN NEW.status = 'SENDING'
  ...
  AND (SELECT email_dispatch_paused FROM outbox_authority WHERE singleton = 1) = 1
BEGIN
...
BEFORE UPDATE ON email_outbox
WHEN (SELECT attempt_authority FROM outbox_authority WHERE singleton = 1) = 'ATTEMPT'
  AND (
