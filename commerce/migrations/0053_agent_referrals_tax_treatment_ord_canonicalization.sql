-- PR-F: tax/VAT treatment authority + provider-neutral ORD canonicalization.
--
-- Two authorities, deliberately never merged into one:
--   legal_profile_revision  - WHO the contractor is (PR-D/D2/PR-E)
--   tax_treatment_revision  - HOW that contractor's income is taxed, which
--                             can change (a USN rate election, a VAT
--                             exemption threshold crossed, ...) WITHOUT the
--                             underlying legal identity changing at all.
-- Extending canonicalLegalProfileEquals() or the legal-profile revision
-- chain itself to also carry VAT facts would conflate the two and force a
-- full D2 supersession cycle (with its own blocking-predicate gate) just to
-- record a rate change - tax_treatment_revisions is its own temporal,
-- append-only chain instead, keyed to an EXACT legal_profile_revision_id
-- but never inherited across a legal-profile supersession.
--
-- Deliberately additive, not a fourth FK-off migration: neither new column
-- on reward_settlements/ord_paid_invoice_payloads is NOT NULL (both tables
...
-- reward_settlements: the settlement-time tax snapshot. Nullable at the
-- column level (this table still carries pre-PR7 LEGACY rows under one
-- schema), but the authority-tuple trigger below makes it mandatory for
-- every genuine AGENT_REFERRALS row from this migration forward.
ALTER TABLE reward_settlements ADD COLUMN tax_treatment_revision_id_snapshot TEXT REFERENCES agent_referrals_tax_treatment_revisions(id);
ALTER TABLE reward_settlements ADD COLUMN tax_canonicalization_version TEXT;
ALTER TABLE reward_settlements ADD COLUMN tax_canonical_json TEXT;
ALTER TABLE reward_settlements ADD COLUMN tax_canonical_hash TEXT;
...
WHEN NOT (
  ...
  (NEW.settlement_flow = 'AGENT_REFERRALS'
    ...
    -- The pinned tax treatment must itself describe the SAME pinned legal
    -- profile and the SAME partner - never a treatment for a different
    -- revision or a different partner's own identity.
    AND EXISTS (
...
-- ord_paid_invoice_payloads: the immutable ORD participant + tax canonical
-- snapshot, pinned at mint time from the settlement's own frozen facts -
-- never re-resolved from current mutable state on replay.
ALTER TABLE ord_paid_invoice_payloads ADD COLUMN tax_treatment_revision_id_snapshot TEXT REFERENCES agent_referrals_tax_treatment_revisions(id);
ALTER TABLE ord_paid_invoice_payloads ADD COLUMN ord_participant_canonicalization_version TEXT;
ALTER TABLE ord_paid_invoice_payloads ADD COLUMN partner_participant_json TEXT;
ALTER TABLE ord_paid_invoice_payloads ADD COLUMN partner_participant_hash TEXT;
ALTER TABLE ord_paid_invoice_payloads ADD COLUMN tax_canonicalization_version TEXT;
ALTER TABLE ord_paid_invoice_payloads ADD COLUMN tax_canonical_json TEXT;
ALTER TABLE ord_paid_invoice_payloads ADD COLUMN tax_canonical_hash TEXT;
...
-- seven new PR-F snapshot columns - a payload's canonical facts, once
-- minted, never drift even under raw UPDATE.
DROP TRIGGER ord_paid_invoice_payloads_authority_immutable_guard;
