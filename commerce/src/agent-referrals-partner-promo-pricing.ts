...
export type ResolvedPromoTerms = { discount_type: "NONE" | "PERCENT" | "FIXED"; discount_value: number };

/**
 * `notEligibleCode` lets the two existing call sites keep their own
 * distinct, already-established codes (PROMO_NOT_ELIGIBLE at quote time,
 * PROMO_NO_LONGER_ELIGIBLE at checkout) rather than this module inventing
 * a third. Never resolves or snapshots an authorization id - quotes are
 * deliberately not eligibility authority (A4-6), and eligibility is
 * re-resolved fresh at every call.
 */
export const resolveCheckoutPromoTerms = (
  db: Database.Database,
  promo: { id: string; discount_type: string; discount_value: number },
  occurrenceId: string,
  notEligibleCode: string,
): ResolvedPromoTerms => {
  if (!isPromoPartnerOwned(db, promo.id)) {
    return { discount_type: promo.discount_type as ResolvedPromoTerms["discount_type"], discount_value: Number(promo.discount_value) };
  }
