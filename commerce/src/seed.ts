...
const cityNames: Record<(typeof CITY_SLUGS)[number], string> = {
  kemerovo: "Кемерово", novosibirsk: "Новосибирск", novokuznetsk: "Новокузнецк", tomsk: "Томск", irkutsk: "Иркутск", krasnoyarsk: "Красноярск",
};
const sqlite = openDatabase(); migrate(sqlite);
const insert = sqlite.prepare("INSERT OR IGNORE INTO cities(id, slug, title) VALUES (?, ?, ?)");
for (const slug of CITY_SLUGS) insert.run(randomUUID(), slug, cityNames[slug]);
