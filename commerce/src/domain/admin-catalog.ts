...
export const createAgent = (host: AdminCatalogHost, input: Record<string, unknown>) => {
  const agentId = id();
  host.db.prepare(`INSERT INTO agents(id, slug, display_name, email, enabled, default_reward_type, default_reward_value)
    VALUES (?, ?, ?, ?, ?, 'PERCENT', 0)`)
    .run(agentId, input.slug, input.display_name, String(input.email).toLowerCase(), input.enabled === false ? 0 : 1);
  return one(host.db, "SELECT * FROM agents WHERE id = ?", agentId)!;
};
...
export const patchAgent = (host: AdminCatalogHost, agentId: string, input: Record<string, unknown>) => {
  const existing = one(host.db, "SELECT * FROM agents WHERE id = ?", agentId);
  ...
  host.db.prepare(`UPDATE agents SET ${fields.map((field) => `${field} = ?`).join(", ")}, updated_at = ? WHERE id = ?`).run(...fields.map((field) => field === "enabled" ? Number(input[field]) : field === "email" ? String(input[field]).toLowerCase() : input[field]), now(), agentId);
  return one(host.db, "SELECT * FROM agents WHERE id = ?", agentId)!;
};

export const createPromo = (host: AdminCatalogHost, input: Record<string, unknown>, promoId: string = id()) => {
  if (input.agent_id && !one(host.db, "SELECT id FROM agents WHERE id = ?", input.agent_id)) throw new DomainError("AGENT_NOT_FOUND", 404);
