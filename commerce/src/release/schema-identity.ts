...
export const classifySchemaLineage = (snapshot: SchemaIdentitySnapshot): SchemaLineage => {
  ...
  // A ledger table with nothing built beside it is a bootstrap that has not
  // happened, not a pre-launch database. Reading it as LEGACY would brick a
  ...
  // on. A real pre-launch database carries a hundred tables beside its ledger.
  if (tables.size === 1 && tables.has("schema_migrations")) return "EMPTY_BOOTSTRAPPABLE";
