...
import type { HistoricalCandidate, ReleaseCandidate, ReleaseCandidateReader, ReleaseClass } from "./candidate";
...
export const canonicalCandidate = (candidate: HistoricalCandidate): string => JSON.stringify([
...
export const candidateDigest = (candidate: HistoricalCandidate): string =>
  createHash("sha256").update(canonicalCandidate(candidate)).digest("hex");

export const assertCandidate = (candidate: ReleaseCandidate): void => assertRecord(candidate, RELEASE_CLASSES);

const assertRecord = (candidate: HistoricalCandidate, classes: readonly HistoricalCandidate["releaseClass"][]): void => {
  ...
  if (!classes.includes(candidate.releaseClass)) problems.push("releaseClass");
...
export class FileReleaseCandidateStore implements ReleaseCandidateReader {
  ...
  publish(candidate: ReleaseCandidate): { readonly candidate: ReleaseCandidate; readonly republished: boolean } {
    ...
    try {
      ...
      const existing = this.readHistorical(candidate.id);
      ...
      return { candidate, republished: true };
    }
  ...
  /**
   * A releasable candidate. A launch candidate is history, not a release, and
   * is refused by name rather than returned as something a caller could act on.
   */
  get(id: string): ReleaseCandidate | undefined {
    const record = this.readHistorical(id);
    if (record?.releaseClass === "LAUNCH_BASELINE") throw new CandidateStoreError("LAUNCH_BASELINE_RETIRED", id);
    return record as ReleaseCandidate | undefined;
  }

  /** Any published candidate, retired classes included, validated but never deployable. */
  readHistorical(id: string): HistoricalCandidate | undefined {
    ...
    const candidate: HistoricalCandidate = {
      ...
      releaseClass: parsed.releaseClass as HistoricalCandidate["releaseClass"],
      expectation: {
    ...
    assertRecord(candidate, HISTORICAL_CLASSES);
