...
const SETTLEMENT_COLUMNS = `id, agent_id, occurrence_id, amount_kopecks, status, settlement_flow, engagement_id, engagement_revision_id,
  base_registry_snapshot_id, effective_reward_snapshot_id, partner_identity_id, payout_profile_revision_id, tax_mode_snapshot,
...
const mintAgentReferralsSettlement = (
  ...
  db.prepare(`INSERT INTO reward_settlements(
    ...
    .run(
      ...
      context.effective.engagement_id, context.effective.engagement_revision_id, context.effective.base_registry_snapshot_id, context.effective.id,
  ...
  const run = db.transaction((): CorrectPartnerRewardResult => {
    ...
    if (existingSettlement) {
      ...
      if (madeAttempt) {
        ...
        return { correction, settlement_action: "RECOVERY_EXPOSURE", exposure: recoveryExposure(db, engagementId) };
      }
