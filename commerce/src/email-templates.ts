...
export function renderEmailTemplate(template: string, payload: Record<string, unknown>) {
  ...
  if (template === "occurrence-cancelled") return { subject: "Мастер-класс отменён", plaintext: `Мастер-класс отменён. Мы оформили полный возврат; срок зачисления зависит от банка.${orderReference(payload)}`, html: `<p>Мастер-класс отменён.</p><p>Мы оформили полный возврат; срок зачисления зависит от банка.</p>${payload.public_order_number ? `<p>Номер заказа: ${escapeHtml(payload.public_order_number)}</p>` : ""}` };
  ...
  if (template === "customer-refund-confirmed") return { subject: "Отмена участия подтверждена", plaintext: `Отмена участия подтверждена. Мы оформили полный возврат; срок зачисления зависит от банка.${orderReference(payload)}`, html: `<p>Отмена участия подтверждена.</p><p>Мы оформили полный возврат; срок зачисления зависит от банка.</p>${payload.public_order_number ? `<p>Номер заказа: ${escapeHtml(payload.public_order_number)}</p>` : ""}` };
  if (template === "refund-succeeded") return { subject: "Возврат оформлен", plaintext: `Возврат оформлен. Срок зачисления зависит от вашего банка.${orderReference(payload)}`, html: `<p>Возврат оформлен. Срок зачисления зависит от вашего банка.</p>${payload.public_order_number ? `<p>Номер заказа: ${escapeHtml(payload.public_order_number)}</p>` : ""}` };
