...
describe("commerce HTTP boundary", () => {
  ...
  it("exposes only authenticated, redacted certification evidence", async () => {
    ...
    expect(await system.json()).toMatchObject({ source_commit: process.env.SOURCE_COMMIT, migration_head: { version: "0033_runtime_release_evidence.sql" }, active_legal_release: { version: "test" } });
