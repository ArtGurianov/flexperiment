...
import { readLaunchCatalogue } from "../../src/launch-seed";
import { loadCanonicalLegalRelease } from "../../src/legal-release";
import { canonicalLegalManifest } from "../../src/legal-manifest";
 ...
 */
/** The same VPS-shaped world both cases below drive. */
const bootstrapWorld = async () => {
  ...
  return {
    root, archive, databasePath, target, config,
    options: {
      now: () => now, fetch: fetch as typeof globalThis.fetch,
      runtimeControl: { async stop() {}, async assertStopped() {}, async start() {} },
      openHandles: { async assertNoOpenHandles() {} },
    },
    async close() {},
  };
};
...
describe("bootstrap storage through the production composition root", () => {
  it("closes the legacy gate, takes online backups, stops both writers, then writes envelope before rename and db.ts bootstrap", async () => {
    ...
    try {
      ...
      try {
        expect(classifySchemaLineage(readSchemaIdentity(launched))).toBe("SUPPORTED");
        // A migrated database is not a servable one. The third cutover attempt
        // reached convergence with zero cities and no legal release, which
        // readiness refuses and certification cannot use.
        const catalogue = readLaunchCatalogue();
        expect((launched.prepare("SELECT COUNT(*) n FROM cities").get() as { n: number }).n).toBe(catalogue.cities.length);
        const legal = activeLegalBinding(launched)!;
        const expected = loadCanonicalLegalRelease();
        expect(legal.version).toBe(expected.version);
        expect(legal.manifestSha256).toBe(createHash("sha256").update(canonicalLegalManifest(expected.manifest)).digest("hex"));
      } finally { launched.close(); }
      expect(release.envelopes.read("cutover-1")?.predecessorDatabase.sha256).toMatch(/^[a-f0-9]{64}$/);

      // The runner is root in production and the runtime is not. A successor
      // the runtime cannot open crash-loops, and because topology is read from
      // that runtime, the recovery path stalls with it.
      const predecessorStat = statSync(join(archive, "cutover-1.predecessor.sqlite"));
      const successorStat = statSync(databasePath);
      expect(successorStat.uid).toBe(predecessorStat.uid);
      expect(successorStat.gid).toBe(predecessorStat.gid);
      expect(successorStat.mode & 0o7777).toBe(predecessorStat.mode & 0o7777);
      // Any sidecar root left behind is handed over with the main file: one
      // owned by root beside a handed-over database is a writer the runtime
      // cannot open.
      for (const sidecar of ["-wal", "-shm"]) {
        if (!existsSync(`${databasePath}${sidecar}`)) continue;
        const stat = statSync(`${databasePath}${sidecar}`);
        expect(stat.uid).toBe(predecessorStat.uid);
        expect(stat.mode & 0o7777).toBe(predecessorStat.mode & 0o7777);
      }
