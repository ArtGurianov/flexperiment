 ...
 *
 * RECONSTRUCTION_BOUND exists for a detached candidate whose SHA is
 * deliberately NOT required to be a main ancestor - the Agent Referrals
 * candidate Q (see commerce/src/agent-referrals-candidate.ts and the plan's
 * §B-1) and, as of the release-semantics bootstrap
 * (docs/release/RELEASE_SEMANTICS_BOOTSTRAP.md), the bootstrap candidate B2
 * (see commerce/src/controlled-candidate.ts, the generic reconstruction core
 * both share). Unlike hardBoundRecoveryTarget above, admission here is a
 * POSITIVE proof: the workflow source must contain all six required
 * assertions, and deleting any single one must break this test. This is
 * deliberately not another hardBoundRecoveryTarget-style skip and not a
 * name-based exemption - `if (workflow.includes("agent-referrals")) skip` is
 * exactly the failure shape this machinery exists to close off.
 *
...
const RECONSTRUCTION_BOUND_ASSERTIONS: ReadonlyArray<{ readonly name: string; readonly pattern: RegExp; readonly removeLine: (source: string) => string }> = [
  {
    ...
    pattern: new RegExp(`git show "\\$(GITHUB_SHA|CONTROLLER_SHA):\\.release/controlled-candidates/(?:${CERTIFICATE_NAMESPACE_SLUGS.join("|")})-\\$BASE_SHA/certificate\\.json" > candidate-certificate\\.json`),
    removeLine: removeLinesMatching(new RegExp(`git show "\\$(GITHUB_SHA|CONTROLLER_SHA):\\.release/controlled-candidates/(?:${CERTIFICATE_NAMESPACE_SLUGS.join("|")})-\\$BASE_SHA/certificate\\.json" > candidate-certificate\\.json`)),
  },
  {
  ...
  },
  {
    ...
    pattern: new RegExp(`node --import tsx commerce/src/(?:${CANDIDATE_VERIFY_SCRIPTS.join("|")})\\.ts`),
    removeLine: removeLinesMatching(new RegExp(`node --import tsx commerce/src/(?:${CANDIDATE_VERIFY_SCRIPTS.join("|")})\\.ts`)),
  },
  {
    name: "real patches are resolved from the same trusted controller tree that supplied the certificate",
    pattern: new RegExp(`node --import tsx commerce/src/(?:${CANDIDATE_VERIFY_SCRIPTS.join("|")})\\.ts\\s+\\S+\\s+"\\$(GITHUB_SHA|CONTROLLER_SHA)"`),
    removeLine: removeLinesMatching(new RegExp(`node --import tsx commerce/src/(?:${CANDIDATE_VERIFY_SCRIPTS.join("|")})\\.ts`)),
  },
  {
...
describe("RECONSTRUCTION_BOUND: positive proof obligation for a detached candidate", () => {
   ...
   * "reconstructs AND deploys it" - that is DEPLOYING's job, proven in the
   * describe block below ("deployment target classification is exhaustive
  ...
  it("accepts a fixture containing all six required assertions", () => {
  ...
  it("does not admit a name-based skip or a new hard-coded exemption in place of the real proof", () => {
    ...
    // with none of the required assertions actually present.
