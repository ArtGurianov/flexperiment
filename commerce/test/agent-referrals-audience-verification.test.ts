...
describe("audience verification: append-only, VERIFIED | REVOKED, no SUPERSEDED state", () => {
  it("mints VERIFIED as revision 1 with no supersedes_event_id", () => {
    const { db } = fresh();
    const { partnerId, cityId } = seedPartnerAndCity(db);
    const event = mintAudienceVerificationEvent(db, admin, partnerId, cityId, "VERIFIED", "initial check", "ev-1");
  ...
  it("refuses REVOKED when there is nothing currently VERIFIED", () => {
    const { db } = fresh();
    const { partnerId, cityId } = seedPartnerAndCity(db);
    expect(() => mintAudienceVerificationEvent(db, admin, partnerId, cityId, "REVOKED", "x", "ev")).toThrow(AudienceVerificationError);
    expect(() => mintAudienceVerificationEvent(db, admin, partnerId, cityId, "REVOKED", "x", "ev")).toThrow(/AGENT_REFERRALS_AUDIENCE_NOT_VERIFIED/);
  });

  it("REVOKED after VERIFIED supersedes it and becomes current; a replacement VERIFIED can follow", () => {
    const { db } = fresh();
    const { partnerId, cityId } = seedPartnerAndCity(db);
    const v1 = mintAudienceVerificationEvent(db, admin, partnerId, cityId, "VERIFIED", "initial", "ev-1");
    const r1 = mintAudienceVerificationEvent(db, admin, partnerId, cityId, "REVOKED", "compliance issue", "ev-2");
    ...
    const v2 = mintAudienceVerificationEvent(db, admin, partnerId, cityId, "VERIFIED", "re-verified with updated evidence", "ev-3");
  ...
  it("every event ever written is exactly VERIFIED or REVOKED - there is no SUPERSEDED state anywhere in the lineage", () => {
    const { db } = fresh();
    const { partnerId, cityId } = seedPartnerAndCity(db);
    mintAudienceVerificationEvent(db, admin, partnerId, cityId, "VERIFIED", "a", "1");
    mintAudienceVerificationEvent(db, admin, partnerId, cityId, "REVOKED", "b", "2");
    mintAudienceVerificationEvent(db, admin, partnerId, cityId, "VERIFIED", "c", "3");
  ...
  it("a different (partner, city) pair has its own independent revision lineage", () => {
    ...
    mintAudienceVerificationEvent(db, admin, p1, c1, "VERIFIED", "a", "1");
    mintAudienceVerificationEvent(db, admin, p1, c1, "REVOKED", "b", "2");
    const c2Event = mintAudienceVerificationEvent(db, admin, p1, c2, "VERIFIED", "c", "1");
  ...
  describe("concurrency: two writers racing to mint the next revision for the same (partner, city)", () => {
    it("both eventually mint, serialized onto distinct revisions - never a duplicate or lost revision", () => {
      ...
      mintAudienceVerificationEvent(a, admin, partnerId, cityId, "VERIFIED", "racer A initial", "ev-a1");
      ...
      const revoked = mintAudienceVerificationEvent(b, admin, partnerId, cityId, "REVOKED", "racer B revokes", "ev-b1");
      const reverified = mintAudienceVerificationEvent(a, admin, partnerId, cityId, "VERIFIED", "racer A re-verifies", "ev-a2");
