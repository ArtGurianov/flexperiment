...
describe("partner authorization: the plan's may/may-not matrix", () => {
  describe("partner MAY", () => {
    it("submit their own legal profile", () => {
      ...
      expect(() => submitPartnerLegalProfile(db, asPartner(partner_identity_id), "INDIVIDUAL", "NPD", { full_name: "Ivanov Ivan Ivanovich", inn: "123456789012" }, 0)).not.toThrow();
    });
  ...
  describe("partner MAY NOT", () => {
    it("verify their own legal profile - verifyPartnerLegalProfile takes only AdminPrincipal", () => {
      const db = fresh();
      const { partner_identity_id } = provisionedActive(db);
      submitPartnerLegalProfile(db, asPartner(partner_identity_id), "INDIVIDUAL", "NPD", { full_name: "Ivanov Ivan Ivanovich", inn: "123456789012" }, 0);
