...
import {
  ...
  AgentReferralsLegalProfileSupersessionError, currentLegalProfileRevisionForPartner, legalProfileChangeRequestHeadForPartner } from "../src/agent-referrals-legal-profile-supersession";
...
describe("D2: legal-profile supersession suspension-policy wiring", () => {
  it("submit and reject are permitted under SUSPENDED (evidence, not authority), verify is not", () => {
    ...
    expect(() => submitLegalProfileSupersession(db, admin, p1.partnerIdentityId, { legalForm: "LEGAL_ENTITY", taxMode: "OTHER", ...legalEntityRequisites, reason: "became org", evidenceRef: "ev.pdf", expectedCurrentLegalProfileRevision: currentLegalProfileRevisionForPartner(db, p1.partnerIdentityId), expectedRequestSequence: legalProfileChangeRequestHeadForPartner(db, p1.partnerIdentityId) })).not.toThrow();
...
describe("D2: submit()", () => {
  ...
  it("two sequential submits for the same identity (serialized by better-sqlite3's synchronous execution): exactly one succeeds, the other gets ALREADY_PENDING via the pre-check, never 500", () => {
    ...
    try { submitLegalProfileSupersession(db, admin, p1.partnerIdentityId, { legalForm: "LEGAL_ENTITY", taxMode: "OTHER", ...legalEntityRequisites, reason: "A", evidenceRef: "ev.pdf", expectedCurrentLegalProfileRevision: currentLegalProfileRevisionForPartner(db, p1.partnerIdentityId), expectedRequestSequence: legalProfileChangeRequestHeadForPartner(db, p1.partnerIdentityId) }); } catch (e) { firstError = e; }
    try { submitLegalProfileSupersession(b, admin, p1.partnerIdentityId, { legalForm: "INDIVIDUAL_ENTREPRENEUR", taxMode: "OTHER", ...individualEntrepreneurRequisites, reason: "B", evidenceRef: "ev.pdf", expectedCurrentLegalProfileRevision: currentLegalProfileRevisionForPartner(b, p1.partnerIdentityId), expectedRequestSequence: legalProfileChangeRequestHeadForPartner(b, p1.partnerIdentityId) }); } catch (e) { secondError = e; }
  ...
  it("the structural backstop itself: a genuine race window (this connection's own pre-check already passed, then a second writer commits first) still resolves via the partial unique index, with the exact error shape submit()'s catch block translates - proving that regex is not merely aspirational", () => {
    ...
    submitLegalProfileSupersession(b, admin, p1.partnerIdentityId, { legalForm: "LEGAL_ENTITY", taxMode: "OTHER", ...legalEntityRequisites, reason: "B", evidenceRef: "ev.pdf", expectedCurrentLegalProfileRevision: currentLegalProfileRevisionForPartner(b, p1.partnerIdentityId), expectedRequestSequence: legalProfileChangeRequestHeadForPartner(b, p1.partnerIdentityId) });
  ...
  it("assertion_source and created_by are derived from the principal's own realm, never accepted from the caller", () => {
    const db = fresh();
    const p1 = readyPartner(db);
    const adminRequest = submitLegalProfileSupersession(db, admin, p1.partnerIdentityId, { legalForm: "LEGAL_ENTITY", taxMode: "OTHER", ...legalEntityRequisites, reason: "admin claim", evidenceRef: "egrul.pdf", expectedCurrentLegalProfileRevision: currentLegalProfileRevisionForPartner(db, p1.partnerIdentityId), expectedRequestSequence: legalProfileChangeRequestHeadForPartner(db, p1.partnerIdentityId) });
    ...
    const partnerRequest = submitLegalProfileSupersession(db, p1.partner, p1.partnerIdentityId, { legalForm: "LEGAL_ENTITY", taxMode: "OTHER", ...legalEntityRequisites, reason: "partner claim", expectedCurrentLegalProfileRevision: currentLegalProfileRevisionForPartner(db, p1.partnerIdentityId), expectedRequestSequence: legalProfileChangeRequestHeadForPartner(db, p1.partnerIdentityId) });
...
describe("D2: replay and terminal-state handling", () => {
  it("verify(REJECTED) and verify(STALE) return INVALID_STATE, read-only", () => {
    const db = fresh();
    const p1 = readyPartner(db);
    const rejected = submitLegalProfileSupersession(db, admin, p1.partnerIdentityId, { legalForm: "LEGAL_ENTITY", taxMode: "OTHER", ...legalEntityRequisites, reason: "x", evidenceRef: "e.pdf", expectedCurrentLegalProfileRevision: currentLegalProfileRevisionForPartner(db, p1.partnerIdentityId), expectedRequestSequence: legalProfileChangeRequestHeadForPartner(db, p1.partnerIdentityId) });
...
describe("D2: submit() proves a PARTNER principal is authority only for its own identity (P1 review fix)", () => {
  ...
  it("a partner submitting for its own identity is unaffected by this check", () => {
    const db = fresh();
    const p1 = readyPartner(db);
    const request = submitLegalProfileSupersession(db, p1.partner, p1.partnerIdentityId, { legalForm: "LEGAL_ENTITY", taxMode: "OTHER", ...legalEntityRequisites, reason: "x", expectedCurrentLegalProfileRevision: currentLegalProfileRevisionForPartner(db, p1.partnerIdentityId), expectedRequestSequence: legalProfileChangeRequestHeadForPartner(db, p1.partnerIdentityId) });
