...
const TARGET = "b4e350f4ba6ecbaf110461dd5356c2293b3e5c2b";
...
describe("controlled Agent Referrals Phase 1 deploy", () => {
  it("is manual-only and binds the reviewed candidate without deriving it", () => {
    ...
    expect(source).toContain('[[ "$(git rev-parse "${TARGET_SHA}^")" == "$BASE_SHA" ]]');
  ...
  it("refuses any gate evidence that is not a literal zero-row production proof", () => {
    ...
    const gates = stepBody("Prove both read-only production gates before any effect");
    expect(gates).not.toMatch(/UPDATE |INSERT |DELETE |backfill/i);
  });

  it("runs both gates before anything that can affect production", () => {
    const gates = stepAt("Prove both read-only production gates before any effect");
...
describe("Phase 1 readiness retry boundary (executed)", () => {
  ...
  const runDeployStep = ({ readinessExit, deploymentState = "FRESH" }: { readinessExit: number; deploymentState?: string }) => {
    ...
    return { result, deploys: lines(deployLog).length, readinessRuns: lines(readinessLog).length };
  };
