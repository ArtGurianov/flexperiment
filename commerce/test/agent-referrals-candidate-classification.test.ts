...
describe("candidate classification is a partition of the exact endpoint diff", () => {
  ...
  /**
   * The check that makes the OTP class of defect unrepeatable.
   *
   * commerce/src/agent-referrals-otp.ts and server.ts carry PRODUCTION-ONLY
   * delivery wiring - UnisenderOtpSender, otpSenderFromEnvironment, and the
   * createApp call that passes it - which BASE has and the frozen source main
   * does not. Taking the source blob whole would have compiled perfectly and
   * silently removed the partner's ability to receive a login code.
   *
   * Feature ownership does not prove the source blob is the right RESULT,
   * because BASE is not an older main: it carries production-only changes.
   * So any certified WHOLE_FILE modification that drops an export BASE had
   * must carry a written reconciliation, and this refuses one that does not.
   */
  it("every certified WHOLE_FILE modification that drops a BASE export carries a recorded reconciliation", () => {
    const exported = (ref: string, path: string): Set<string> => {
      const shown = spawnSync("git", ["show", `${ref}:${path}`], { encoding: "utf8", maxBuffer: 1024 * 1024 * 64 });
      if (shown.status !== 0) return new Set();
      return new Set([...shown.stdout.matchAll(/^export (?:const|class|function|type|interface|enum) (\w+)/gm)].map((match) => match[1]));
    };

    const unreconciled: string[] = [];
    for (const row of rows) {
      if (row.disposition !== "CERTIFIED" || row.category !== "WHOLE_FILE" || row.status !== "M") continue;
      if (!/\.tsx?$/.test(row.path)) continue;
      const lost = [...exported(baseSha, row.path)].filter((name) => !exported(sourceMainSha, row.path).has(name));
      if (lost.length > 0 && !row.reconciliation?.trim()) {
        unreconciled.push(`${row.path} drops ${lost.join(", ")}`);
      }
    }
    expect(unreconciled, "a WHOLE_FILE path may not silently drop production behaviour").toEqual([]);
  });
