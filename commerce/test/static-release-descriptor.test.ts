...
describe("static release descriptors", () => {
  ...
  it("writes the supplied immutable frontend source commit into the static artifact", () => {
    ...
    expect(readDescriptor(output)).toEqual({ source_commit: sourceCommit, checkout_contract_version: "promo-codes-v0" });
  });
  ...
  it("writes the supplied immutable admin source commit into the static artifact", () => {
    ...
    expect(readDescriptor(output)).toEqual({ source_commit: sourceCommit, admin_contract_version: "promo-codes-v0" });
  });
