...
# The certification target is operator-supplied; this historical script must
# not quietly certify an obsolete migration or legal release by default.
: "${EXPECTED_MIGRATION:?EXPECTED_MIGRATION is required}"
: "${EXPECTED_LEGAL_VERSION:?EXPECTED_LEGAL_VERSION is required}"
: "${EXPECTED_LEGAL_RELEASE_ID:?EXPECTED_LEGAL_RELEASE_ID is required}"
: "${EXPECTED_PUBLIC_OFFER_SHA256:?EXPECTED_PUBLIC_OFFER_SHA256 is required}"
: "${EXPECTED_PRIVACY_POLICY_SHA256:?EXPECTED_PRIVACY_POLICY_SHA256 is required}"
: "${EXPECTED_PD_CONSENT_SHA256:?EXPECTED_PD_CONSENT_SHA256 is required}"
: "${EXPECTED_CHECKOUT_DISCLOSURE_SHA256:?EXPECTED_CHECKOUT_DISCLOSURE_SHA256 is required}"
CITY_SLUG="kemerovo"
...
usage() {
  cat <<'EOF'
Usage:
  EXPECTED_MIGRATION=... EXPECTED_LEGAL_VERSION=... EXPECTED_LEGAL_RELEASE_ID=... \\
  EXPECTED_PUBLIC_OFFER_SHA256=... EXPECTED_PRIVACY_POLICY_SHA256=... \\
  EXPECTED_PD_CONSENT_SHA256=... EXPECTED_CHECKOUT_DISCLOSURE_SHA256=... ./certification.sh
EOF
}
