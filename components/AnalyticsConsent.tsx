...
import { ANALYTICS_CONSENT_CHANGE_EVENT, readAnalyticsConsent, storeAnalyticsConsent } from "@/components/analytics-consent-client";
import { browserMetrikaEnvironment, canBootstrapMetrika, createMetrikaManager, metrikaCounterId } from "@/components/metrika";
...
export default function AnalyticsConsent() {
  ...
  useEffect(() => {
    ...
    manager.current ??= createMetrikaManager(counterId, browserMetrikaEnvironment());
    manager.current.enable();
    manager.current.observe(pathname, search ? `?${search}` : "");
  ...
  return (
    <>
      {consent === "UNDECIDED" && (
        ...
        >
          <p>Необязательная аналитика отключена. Разрешите Яндекс Метрике получать только обезличенные просмотры страниц?</p>
      ...
      {settingsOpen && (
        <section role="dialog" aria-modal="true" aria-label="Настройки cookies" className="fixed inset-0 z-[121] grid place-items-end bg-ink/75 p-3 sm:place-items-center">
          <div className="w-full max-w-md border-2 border-acid bg-ink p-5 font-mono text-sm text-bone shadow-[5px_6px_0_var(--color-shadow)]">
            ...
            <p className="mt-3">Необязательная аналитика передаёт только безопасные адреса страниц. Вы можете изменить выбор в любое время.</p>
