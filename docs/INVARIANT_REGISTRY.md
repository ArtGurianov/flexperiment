...
| The production deploy pointer moves only by compare-and-set | `commerce/src/release/deploy-ref.ts`. `ProductionDeployRefStore.compareAndSet` pushes under `--force-with-lease`, proves the target resolves on the remote before pushing, and reads the pointer back afterwards - a push the remote accepted while the ref ended up elsewhere is refused as `DEPLOY_REF_NOT_MOVED` rather than reported as a move. The pointer is also the control-plane half of `PreDeploySnapshot`, so a safe abort has to prove it came home too. Proved in `release/deploy-ref.test.ts` against a real git remote and in `release/deploy-session.test.ts`. |
