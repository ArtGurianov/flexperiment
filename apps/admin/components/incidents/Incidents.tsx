  ...
  return (
    <>
      ...
      <section className="panel">
        ...
        <p className="notice">Закрытие этой записи — только bookkeeping review. Статус payment/refund меняется только на основании provider evidence.</p>
        {drift.isLoadingError ? <Notice error={(drift.error as { code?: string } | null)?.code ?? "UNKNOWN"} /> : !drift.data ? <Loading /> : drift.data.reviews.length ? <table><thead><tr><th>Создан</th><th>Сущность</th><th>Evidence</th><th /></tr></thead><tbody>{drift.data.reviews.map((review) => <tr key={string(review.id)}><td>{formatDate(review.created_at)}</td><td><strong>{string(review.entity_type)}</strong><small>{string(review.entity_id)}</small>{review.refund_id ? <small>Refund: {string(review.refund_source) || string(review.refund_id)}</small> : null}{review.order_id ? <small><Link href={`/orders?id=${encodeURIComponent(string(review.order_id))}`}>Открыть заказ {string(review.public_order_number) || "и payment evidence"}</Link></small> : null}</td><td><code>{string(review.observed_json)}</code></td><td><button onClick={() => setResolvingDrift(review)}>Закрыть review</button></td></tr>)}</tbody></table> : <Empty label="Открытых provider drift review пока нет." />}
      </section>
