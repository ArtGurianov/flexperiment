...
function Orders() {
  ...
  return <><PageTitle eyebrow="COMMERCE / ORDERS" title={<>Заказы<br /><i>и их evidence.</i></>} text="Payment, booking, ticket, email и refund остаются отдельными фактами. Экран не сворачивает их в одну фиктивную метку." />
    <section className="panel"><div className="filters"><label>Город<select value={filters.city_id} onChange={(event) => setFilters((previous) => ({ ...previous, city_id: event.target.value }))}><option value="">Все</option>{cities.value?.cities.map((city) => <option key={string(city.id)} value={string(city.id)}>{string(city.title)}</option>)}</select></label><label>Событие<select value={filters.occurrence_id} onChange={(event) => setFilters((previous) => ({ ...previous, occurrence_id: event.target.value }))}><option value="">Все</option>{occurrences.value?.occurrences.map((occurrence) => <option key={string(occurrence.id)} value={string(occurrence.id)}>{string(occurrence.title)}</option>)}</select></label><label>Payment<select value={filters.payment_status} onChange={(event) => setFilters((previous) => ({ ...previous, payment_status: event.target.value }))}><option value="">Все</option>{["PENDING", "PAID", "PARTIALLY_REFUNDED", "REFUNDED", "REVIEW_REQUIRED"].map((value) => <option key={value}>{value}</option>)}</select></label><label>Booking<select value={filters.booking_status} onChange={(event) => setFilters((previous) => ({ ...previous, booking_status: event.target.value }))}><option value="">Все</option>{["RESERVED", "CONFIRMED", "CANCELLED"].map((value) => <option key={value}>{value}</option>)}</select></label></div>{orders.loading ? <Loading /> : orders.value ? <table><thead><tr><th>Дата</th><th>Заказ</th><th>Событие</th><th>Заказчик</th><th>Участник</th><th>Сумма</th><th>Состояния</th></tr></thead><tbody>{orders.value.orders.map((order) => <tr className="click-row" onClick={() => { setSelected(string(order.id)); history.replaceState(null, "", `/orders/?id=${encodeURIComponent(string(order.id))}`); }} key={string(order.id)}><td>{formatDate(order.created_at)}</td><td><strong>{string(order.public_order_number) || `${string(order.public_status_id).slice(0, 11)}…`}</strong><small>{string(order.id)}</small></td><td>{string(order.city_title)}<small>{string(order.occurrence_title)}</small></td><td>{string(order.customer_name)}<small>{string(order.customer_email)}</small></td><td>{string(order.participant_name) || string(order.customer_name)}<small>{order.participant_age_at_occurrence === null || order.participant_age_at_occurrence === undefined ? "Возраст legacy: неизвестен" : `${number(order.participant_age_at_occurrence)} лет`}</small>{order.participant_requires_adult_accompaniment === 1 && <Badge>ТРЕБУЕТСЯ СОПРОВОЖДЕНИЕ</Badge>}</td><td>{formatMoney(order.amount_kopecks)}</td><td><Badge>{string(order.payment_state)}</Badge><Badge>{string(order.payment_status)}</Badge><Badge>{string(order.booking_status)}</Badge></td></tr>)}</tbody></table> : <Notice error={orders.error} />}</section>{selected && <OrderEvidence id={selected} close={() => { setSelected(null); history.replaceState(null, "", "/orders/"); }} />}</>;
}
...
function EmailAttention() {
  ...
  return <><PageTitle eyebrow="OPERATIONS / EMAIL" title={<>Email<br /><i>attention.</i></>} text="Delivery status — это provider evidence. Acknowledgement означает только review оператором и не отправляет письмо повторно." />
    {data.loading ? <section className="panel"><Loading /></section> : data.value ? <><section className="panel"><h2>Требуют внимания</h2><p className="notice">Требуют внимания: <strong>{data.value.attention_count}</strong></p><EmailIncidentTable incidents={data.value.incidents.filter((incident) => number(incident.requires_attention) === 1)} acknowledge={setAcknowledging} empty="Нет email-инцидентов, требующих внимания." /></section><section className="panel detail"><h2>Рассмотренные</h2><EmailIncidentTable incidents={data.value.incidents.filter((incident) => number(incident.requires_attention) === 0)} empty="Рассмотренных email-инцидентов пока нет." /></section></> : <section className="panel"><Notice error={data.error} /></section>}
...
function OperationalIncidents() {
  ...
  return <><PageTitle eyebrow="OPERATIONS / REVIEW" title={<>Операционные<br /><i>инциденты.</i></>} text="Автоматическая фиксация создаёт review-задачу, но не делает финансовое решение за оператора." />
    <section className="panel">{data.loading ? <Loading /> : data.value ? <><p className="notice">Открыто: <strong>{data.value.open_count}</strong></p>{data.value.incidents.length ? <table><thead><tr><th>Создан</th><th>Тип</th><th>Заказ / покупатель</th><th>Возврат / provider</th><th>Статус</th></tr></thead><tbody>{data.value.incidents.map((incident) => <tr key={string(incident.id)}><td>{formatDate(incident.created_at)}<small>{string(incident.id)}</small></td><td><Badge>{string(incident.kind)}</Badge><small>{string(incident.entity_type)}: {string(incident.entity_id)}</small></td><td>{string(incident.public_order_number) || "—"}<small>{string(incident.customer_email) || "—"}</small></td><td>{incident.refund_public_id ? <><strong>{formatMoney(incident.refund_amount_kopecks)}</strong><small>{string(incident.refund_status)} · {string(incident.refund_public_id)}</small><small>{string(incident.refund_provider_reference) || string(incident.provider_payment_id) || "provider reference отсутствует"}</small><small>{string(incident.refund_last_error) || "—"}</small></> : <small>—</small>}</td><td>{string(incident.status) === "OPEN" ? <button onClick={() => setResolving(incident)}>Отметить решённым</button> : <><Badge>RESOLVED</Badge><small>{string(incident.resolution_note)}</small></>}</td></tr>)}</tbody></table> : <Empty label="Открытых или исторических инцидентов пока нет." />}</> : <Notice error={data.error} />}</section>
