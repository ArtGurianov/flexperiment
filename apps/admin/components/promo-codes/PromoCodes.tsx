...
export function PromoCodes() {
  ...
  return <><PageTitle eyebrow="COMMERCE / DISCOUNTS" title={<>Промо<br /><i>коды.</i></>} text="One global code per checkout. Its price evidence is frozen on every new order." />
    <section className="two-col catalog-grid"><Panel title="Промокоды">{promos.isLoadingError ? <Notice error={(promos.error as { code?: string }).code} /> : !promos.data ? <Loading /> : <table><thead><tr><th>Код</th><th>Скидка</th><th>Агент</th><th /></tr></thead><tbody>{promos.data.promo_codes.map((promo) => <tr key={string(promo.id)}><td><strong>{string(promo.code)}</strong><small>{string(promo.status)}</small></td><td>{string(promo.discount_type) === "NONE" ? "Без скидки" : string(promo.discount_type) === "PERCENT" ? formatBasisPoints(number(promo.discount_value)) : `${(number(promo.discount_value) / 100).toFixed(2)} ₽`}</td><td>{promo.agent && typeof promo.agent === "object" ? string((promo.agent as Row).display_name) : "—"}</td><td><button onClick={() => setEditing(promo)}>Редактировать</button></td></tr>)}</tbody></table>}</Panel>
