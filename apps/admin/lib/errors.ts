...
const PRESENTATIONS: Readonly<Record<string, AdminErrorPresentation>> = {
  ...
  AGENT_REFERRALS_LEGAL_HOLD_ALREADY_ACTIVE: {
    message: "У этого партнёра уже стоит активный правовой запрет на удаление.",
    hint: "Он держится до явного снятия; второй запрет не нужен и не создаётся.",
  },
  AGENT_REFERRALS_PARTNER_PROMO_ALREADY_EXISTS: {
    message: "У этого партнёра уже есть постоянный промокод.",
    hint: "Промокод партнёра выдаётся один раз и не заменяется: он и есть его постоянная идентичность в атрибуции.",
  },
  AGENT_REFERRALS_PARTNER_ALREADY_PROVISIONED: {
    message: "У этого агента уже есть партнёр.",
    hint: "Если предыдущая попытка оборвалась — она прошла. Откройте партнёра в списке; повторно приглашать не нужно.",
  },
