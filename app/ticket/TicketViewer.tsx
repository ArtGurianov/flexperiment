...
export default function TicketViewer() {
  ...
  const [ticket, setTicket] = useState<{ id: string; title: string; starts_at: string; venue_name?: string; venue_address?: string; participant_name: string; requires_adult_accompaniment?: number; status: string } | null>(null);
  useEffect(() => {
  ...
  return <main className="mx-auto flex min-h-dvh max-w-lg items-center px-6 text-center"><div className="w-full border-2 border-acid bg-ink p-8 font-mono text-bone"><h1 className="font-display text-3xl uppercase text-acid">Билет</h1>{result === "loading" && <p className="mt-4">Открываем билет…</p>}{result === "error" && <p className="mt-4">Ссылка на билет недействительна или больше недоступна.</p>}{ticket && <div className="mt-4 space-y-2"><p>Участник: {ticket.participant_name}</p><p>{ticket.title}</p><p>{new Date(ticket.starts_at).toLocaleString("ru-RU")}</p><p>{ticket.venue_name} {ticket.venue_address}</p>{Boolean(ticket.requires_adult_accompaniment) && <p className="border border-acid p-2 text-acid">Требуется сопровождающий совершеннолетний</p>}<p>Идентификатор билета: {ticket.id}</p><p>{ticket.status === "VOID" ? "Билет аннулирован" : "Билет действителен"}</p></div>}</div></main>;
}
